...
class SoundStreamTrainer:
    ...
    def __init__(
        ...
        # Optimizers
        self.g_optimizer = optim.AdamW(model.parameters(), lr=config["g_lr"], betas=(0.8, 0.9))

        self.d_optimizer = optim.AdamW(discriminator.parameters(), lr=config["d_lr"], betas=(0.8, 0.9))

        # Learning rate schedulers (exponential decay)
        self.g_scheduler = optim.lr_scheduler.ExponentialLR(self.g_optimizer, gamma=config.get("lr_decay", 0.999996))

        self.d_scheduler = optim.lr_scheduler.ExponentialLR(self.d_optimizer, gamma=config.get("lr_decay", 0.999996))
    ...
    def train_step(self, batch):
        ...
        fake_logits_list = disc_outputs_for_gen["fake_wave_logits"] + [disc_outputs_for_gen["fake_stft_logits"]]
        real_features_list = disc_outputs_for_gen["real_wave_features"] + [disc_outputs_for_gen["real_stft_features"]]
        fake_features_list = disc_outputs_for_gen["fake_wave_features"] + [disc_outputs_for_gen["fake_stft_features"]]

        # Compute generator loss
        gen_loss, gen_loss_dict = self.gen_loss_fn(
            fake_logits_list, real_features_list, fake_features_list, fake_audio, real_audio
        )
        ...
        if self.global_step >= self.disc_warmup_steps:
            ...
            # Combine all logits
            real_logits_list = disc_outputs["real_wave_logits"] + [disc_outputs["real_stft_logits"]]
            fake_logits_list = disc_outputs["fake_wave_logits"] + [disc_outputs["fake_stft_logits"]]
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default="soundstream",
        choices=["soundstream", "tinystream"],
    ...
    if args.model == "tinystream":
        ...
        print(f"Encoder+Quantizer: {model.get_model_size_mb():.3f} MB")
